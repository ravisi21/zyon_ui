      ...
      <div className="w-full mt-1">
        <div className="flex h-2 rounded-lg overflow-hidden">
          <div
            ...
            style={{ width: `${advPct}%`, transition: "width 0.3s" }}
          />
          <div
            className="bg-red-600"
            style={{ width: `${decPct}%`, transition: "width 0.3s" }}
          />
        </div>
      </div>
    </div>
...
const scriptsList = (data, classes, title) => (
  <Card
    size="small"
    title={title}
    className={`mr-0 md:mr-4 flex flex-col flex-1 ${classes}`}
  ...
  >
    <div className="flex flex-col flex-1">
      {data.map((idx) => (
        <div
          key={idx.scriptId}
          className="flex flex-1 items-center justify-between py-1 border-b border-neutral-700 last:border-none"
        >
          <div className="text-sm md:text-base text-gray-300 font-semibold">
            {idx.scriptId}
          </div>
          <div
            className={`rounded-md p-1 flex md:flex-col items-center justify-center w-[130px] ${idx.change >= 0 ? "bg-price-green" : "bg-price-red"}`}
          >
            <div className="text-white mr-1 font-semibold text-sm">
              {formatPrice(idx.price)}
            </div>
            <div className="text-white text-xs">
              <span className="hidden md:inline">
                {idx.change >= 0 ? "+" : ""}
                {formatPrice(idx.change)}
              </span>
              <span> ({formatPrice(idx.changePct)}%)</span>
            </div>
          </div>
        </div>
      ))}
...
const futuresList = (data, classes, title) => (
  <Card size="small" title={title} className={`mr-0 md:mr-4 ${classes}`}>
    <div className="flex flex-col flex-1">
      {data.map((idx) => (
        <div
          key={idx?.underlyingQuote?.scriptId}
          className="flex flex-1 flex-col items-center justify-between py-1 border-b border-neutral-700 last:border-none"
        >
          <div className="text-base text-white font-semibold py-2">
            {idx?.underlyingQuote?.scriptId}
          </div>
          <div className="flex w-full">
            <div
              className={`p-1 flex flex-col flex-1 items-center justify-center ${idx?.underlyingQuote?.change >= 0 ? "text-price-green" : "text-price-red"}`}
            >
              <div className="mr-1 font-semibold text-sm">
                {formatPrice(idx?.underlyingQuote?.price)}
              </div>
              <div className="text-xs">
                ({idx?.underlyingQuote?.change >= 0 ? "+" : ""}
                {formatPrice(idx?.underlyingQuote?.changePct)}%)
              </div>
            </div>
            <div
              className={`p-1 flex flex-col flex-1 items-center justify-center text-fuchsia-600`}
            >
              <div className="mr-1 font-semibold text-sm">OI Change</div>
              <div className="text-xs">
                ({idx?.oiChange >= 0 ? "+" : ""}
                {formatPrice((idx?.oiChange * 100) / idx?.oi || 0)}%)
              </div>
            </div>
          </div>
        </div>
...
const TopMoversCarousel = ({ data, title, titleClasses }) => {
  ...
  return (
    <div className="mb-4">
      <div className={`font-semibold text-base mb-2 ml-2 ${titleClasses}`}>
        {title}
      </div>
      <Slider {...settings}>
        {data.map((item, idx) => (
...
const TopMoverCarouselCard = ({ scriptId, price, change, changePct }) => {
  ...
  return (
    <div className="bg-black shadow-lg rounded-lg shadow flex flex-col mx-2 relative overflow-hidden">
      <div className="flex-1 flex content-center justify-center text-center text-lg text-neutral-200 p-2">
        {scriptId}
      </div>
      <div
        className={`flex flex-col items-center justify-center flex-1 ${isPositive ? "bg-price-green" : "bg-price-red"}`}
      >
        <div className="flex-1 flex flex-col p-2">
          <div className="font-semibold text-base text-gray-900 mb-1 text-center">
            {formatPrice(typeof price === "object" ? price.parsedValue : price)}
          </div>
          <div className={`text-sm font-semibold text-center `}>
            {isPositive ? "+" : ""}
            {formatPrice(
              typeof change === "object" ? change.parsedValue : change,
            )}
            <span className="ml-1">
              ({isPositive ? "+" : ""}
...
const MarketAnalyticsWidget = forwardRef((props, ref) => {
  ...
  return (
    <div
      style={{ width: "100%", maxWidth: 1400, margin: "0 auto", padding: 12 }}
    >
      ...
      <div className="flex-col flex md:flex-row mb-4">
        ...
        <div className="flex flex-col w-full md:w-auto md:min-w-[300px] md:mr-2">
          <Card
            ...
            className="flex flex-col mb-3 md:mb-0"
          >
      ...
      <div className="mb-2 text-lg font-semibold text-neutral-200">
        Futures Action
      </div>
