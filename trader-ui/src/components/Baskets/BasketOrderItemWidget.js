...
const BasketOrderItemWidget = ({ order, isEditMode, onChange, onDelete }) => {
  ...
  return (
    <li
      className={
        "flex items-center gap-3 py-2 !border-b !border-neutral-700 last:!border-none"
      }
    >
      {isEditMode ? (
        <Switch
          checked={order.isBuy}
          onChange={(v) =>
            onChange(order.tempId || order.basketItemId, "isBuy", v)
          }
        ...
        >
          ...
          />
          <span
            className={`absolute left-2 text-xs font-bold ${order.isBuy ? "text-green-900" : "text-red-900"}`}
          >
            {order.isBuy ? "Buy" : ""}
          </span>
          <span
            className={`absolute right-2 text-xs font-bold ${!order.isBuy ? "text-red-900" : "text-green-900"}`}
          >
            {!order.isBuy ? "Sell" : ""}
          </span>
        </Switch>
      ...
      {/* Script name */}
      <span className="flex-1 text-neutral-200 font-medium text-sm">
        {order.scriptId}
      </span>
      {/* Quantity */}
      {isEditMode ? (
        <CustomStepper
          ...
          onChange={(v) =>
            onChange(order.tempId || order.basketItemId, "quantity", v)
          }
          min={1}
          step={script?.lot || 1}
          style={{ width: 80 }}
