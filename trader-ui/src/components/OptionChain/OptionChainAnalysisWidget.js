...
const OptionChainAnalysisWidget = () => {
  ...
  const [selectedExpiry, setSelectedExpiryState] =
    useState(getSelectedExpiry());
  ...
  const chartData = useMemo(
    () => chainData?.optionChain?.strikes || [],
    [chainData],
  );
  const pcrSeries = useMemo(() => chainData?.pcrSeries || [], [chainData]);
  const timestamps = useMemo(() => chainData?.timestamps || [], [chainData]);
  const callsActiveData = useMemo(
    () => chainData?.callsActiveData || [],
    [chainData],
  );
  const putsActiveData = useMemo(
    () => chainData?.putsActiveData || [],
    [chainData],
  );
  const underlyingPrices = useMemo(
    () => chainData?.underlyingPrices || [],
    [chainData],
  );
  const latestUnderlying = useMemo(
    () => chainData?.underlyingPrice || 0,
    [chainData],
  );
  ...
  const callSeries = useMemo(
    () => callsActiveData.map((call) => call.scriptId),
    [callsActiveData],
  );
  const putSeries = useMemo(
    () => putsActiveData.map((put) => put.scriptId),
    [putsActiveData],
  );
  const hasSeries = useMemo(
    () => callSeries.length > 0 || putSeries.length > 0,
    [callSeries, putSeries],
  );

  const minStrike = useMemo(
    () => (latestUnderlying ? latestUnderlying * 0.95 : -Infinity),
    [latestUnderlying],
  );
  const maxStrike = useMemo(
    () => (latestUnderlying ? latestUnderlying * 1.05 : Infinity),
    [latestUnderlying],
  );
    ...
    () =>
      chartData
        ...
        .map((row) => {
          ...
          const putOiChange =
            row.putOiDiffs && row.putQuote
              ? row.putOiDiffs[oiChangeIndex] || 0
              : row.putQuote?.oiChange || 0;
          const callOiChange =
            row.callOiDiffs && row.callQuote
              ? row.callOiDiffs[oiChangeIndex] || 0
              : row.callQuote?.oiChange || 0;
          return {
            ...row,
            putQuote: row.putQuote
              ? { ...row.putQuote, oiChange: putOiChange }
              : undefined,
            callQuote: row.callQuote
              ? { ...row.callQuote, oiChange: callOiChange }
              : undefined,
          };
        })
        .map((row) => {
          ...
          return {
            ...
            "PE OI":
              (row.putQuote?.oiChange <= 0
                ? row.putQuote?.oi
                : row.putQuote?.oi - row.putQuote?.oiChange) || 0,
            "CE OI":
              (row.callQuote?.oiChange <= 0
                ? row.callQuote?.oi
                : row.callQuote?.oi - row.callQuote?.oiChange) || 0,
  ...
  const seriesColors = ["#DD5555", "#DD9955", "#55DD55", "#55DDAA"];
    ...
    return (
      <div className="bg-dark-bg-2 p-2 rounded shadow text-xs">
        ...
        {payload.map((entry, idx) => {
          ...
          const isOI =
            callSeries.includes(entry.name) || putSeries.includes(entry.name);
  ...
  const CustomBarWithPattern = (props) => {
    ...
    return (
      <rect x={x} y={drawY} width={width} height={drawHeight} fill={barFill} />
    );
  };
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <div className="flex items-center gap-2 ml-4 flex-1 justify-end">
                ...
                <div className="flex items-center">
                  <span className="hidden md:block text-sm text-neutral-400 mr-2">
                    Change in OI since
                  </span>
                  <RadioGroup
                    value={oiChangeIndex}
                    onChange={handleIntervalChange}
                    className="flex gap-1 bg-neutral-800 rounded p-1"
                  >
                    {[
                      { label: "5", value: 0 },
                      { label: "15", value: 1 },
            ...
            <ResponsiveContainer width="100%" aspect={isMobile ? 1.5 : 3}>
              ...
              >
                ...
                />
                <RechartsTooltip
                  cursor={<CenteredCursor />}
                  content={CustomOIBarTooltip}
                />
                <Bar dataKey="PE OI" stackId="pe" fill="#55DD55" />
                <Bar
                  dataKey="PE OI Change"
                  stackId="pe"
                  shape={(props) => (
                    <CustomBarWithPattern {...props} dataKey="PE OI Change" />
                  )}
                />
                <Bar dataKey="CE OI" stackId="ce" fill="#FF4444" />
                <Bar
                  dataKey="CE OI Change"
                  stackId="ce"
                  shape={(props) => (
                    <CustomBarWithPattern {...props} dataKey="CE OI Change" />
                  )}
                />
              </BarChart>
            </ResponsiveContainer>
          ...
          <div className="mb-8">
            ...
            {!hasSeries && (
              <div className="text-red-400">
                No active strike series to plot.
              </div>
            )}
            <ResponsiveContainer width="100%" aspect={isMobile ? 1.5 : 3}>
              <LineChart
              ...
              >
                ...
                {putsActiveData.map((put, idx) => (
                  <Line
                    ...
                    stroke={
                      seriesColors[
                        (idx + callsActiveData.length) % seriesColors.length
                      ]
                    }
                    strokeWidth={1}
                    dot={false}
