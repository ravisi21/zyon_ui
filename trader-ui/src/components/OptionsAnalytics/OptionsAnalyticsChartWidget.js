...
const OptionsAnalyticsChartWidget = React.memo(
  ({ analyticsData, onPriceSelect }) => {
    ...
    const currentProfitDot = useCallback(
      (props) => {
    ...
    const expiryProfitDot = useCallback(
      (props) => {
        const { cx, cy, payload } = props;
        if (payload.price === clickedPrice || payload.price === currentPrice) {
          return (
