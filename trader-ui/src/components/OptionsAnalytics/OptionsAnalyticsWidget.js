...
const OptionsAnalyticsWidget = React.memo(
  React.forwardRef(
    ({ enableScriptSearch = false, allowDeletePositions = false }, ref) => {
      ...
      useEffect(() => {
        ...
        const unsubscribe = eventBus.on(
          EVENT_TYPES.OPTION_ANALYTICS_UPDATE,
          () => setData(),
        );
        return () => {
      ...
      const callOptionAnalysis = useCallback(
        async (
          newDateIndex = null,
          newIvAdjustment = null,
          newPositions = null,
          newUnderlying = null,
        ) => {
          setLoading(true);
          try {
            ...
            const dateIndexToUse =
              newDateIndex !== null ? newDateIndex : selectedDateIndex;
            const ivToUse =
              newIvAdjustment !== null ? newIvAdjustment : ivAdjustment;
            const positionsToUse =
              newPositions || analyticsData?.allItems || [];
            ...
            const dataCopy = JSON.parse(
              JSON.stringify(response.optionAnalysis || response),
            );
      ...
      const handleUnderlyingChange = useCallback(
        (value) => {
          ...
          timeoutRef.current = setTimeout(
            () => callOptionAnalysis(null, null, null, value),
            100,
          );
        },
        [callOptionAnalysis],
      ...
      const createPositionHandlers = useMemo(() => {
        return (idx) => ({
          handleDisabledChange: (e) =>
            handlePositionChange(idx, { disabled: !e.target.checked }),
          handleQuantitySignChange: (checked) => {
            const pos = analyticsData?.allItems[idx];
            if (pos) {
              const newQuantity = checked
                ? Math.abs(pos.netQuantity)
                : -Math.abs(pos.netQuantity);
      ...
      React.useImperativeHandle(ref, () => ({
        async fetchStrategyItems(strategyId, underlying, expiry) {
          ...
          let items = [];
          try {
            const response = await getItemsForStrategy(
              strategyId,
              underlying,
              expiry,
            );
            ...
            <div className="flex-1 mt-3 bg-neutral-900 shadow-lg rounded-lg p-3 mb-3 border border-neutral-700">
                ...
                {analyticsData?.allItems.map((pos, idx) => {
                  ...
                  return (
                    <>
                      <div
                        key={pos.scriptId}
                        className="flex flex-col md:flex-row gap-2 mb-2 bg-gray-50 rounded-md px-2 py-2"
                      >
                        <div className="block md:hidden font-semibold w-full flex-1 text-start text-gray-700 text-sm truncate">
                          {pos.scriptId}
                        </div>
                        <div className="flex items-center flex-1">
                          ...
                          <Switch
                            ...
                            className={
                              pos.netQuantity > 0
                                ? "!bg-blue-600 hover:!bg-blue-600 focus:!bg-blue-600 active:!bg-blue-600"
                                : "!bg-red-600 hover:!bg-red-600 focus:!bg-red-600 active:!bg-red-600"
                            }
                            style={{ minWidth: 40 }}
                          />
                          <div className="hidden md:block w-32 flex-1 text-start text-gray-800 text-sm truncate">
                            {pos.scriptId}
                          </div>
                          ...
                          <CustomStepper
                            ...
                            style={{ width: 70 }}
                          />
                          <div className="w-20 text-right text-gray-700 text-base">
              ...
              {loading && (
                <div className="text-center py-2">
                  <div className="text-sm text-zyon-blue">
                    Updating analysis...
                  </div>
                </div>
              )}
