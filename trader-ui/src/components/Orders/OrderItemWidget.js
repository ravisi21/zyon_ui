...
const OrderItemWidget = ({ order, isLast }) => {
  const [script, setScript] = React.useState(() =>
    scriptsStore.getScript(order.scriptId),
  );
  ...
  const orderTypeColor =
    order.orderType === "BUY" ? "text-blue-400" : "text-price-red";
    ...
    >
      ...
      <div className="flex-1 gap-3">
        ...
        <div className="flex items-center mt-2 gap-2">
          ...
          <span className="text-neutral-500 text-xs flex items-center gap-1">
            <ClockCircleOutlined /> {formatTime(order.lastUpdateTimeMs)}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-end ml-3">
        ...
        <div className="text-neutral-400 text-xxs mt-1">
          ...
          ) : ["sl", "slm"].includes(order.kind?.toLowerCase()) &&
            order.triggerPrice ? (
      ...
      {status === "PENDING" && (
        ...
        >
          <CustomDeleteIcon className="text-neutral-400 text-2xl" />
        </div>
      )}
    </div>
  );
