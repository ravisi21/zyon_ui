  ...
  return (
    <div className="flex flex-col gap-2 bg-dark-bg-2 rounded-lg p-1 mt-3 w-[400px] relative">
      ...
      {liveOrders.map((order, idx) => {
        return (
          <div
            key={order.scriptId + order.orderType + order.orderQuantity + idx}
            className="flex flex-1 items-center gap-4 p-2 rounded bg-dark-bg"
          >
            <span
            ...
            <span className="text-sm text-neutral-200 font-semibold flex-1">
              {order.scriptId}
            </span>
            <span className="text-sm text-neutral-200 w-[50px] text-right">
              {" "}
