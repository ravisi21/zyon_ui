...
const PositionItemWidget = ({ position }) => {
  ...
  const [script, setScript] = useState(() =>
    scriptsStore.getScript(position.scriptId),
  );
  const [quote, setQuote] = useState(() =>
    quotesStore.getQuote(position.scriptId),
  );
    ...
    >
      <div className="flex justify-between items-center py-2 w-full">
        ...
        <div className="flex-1">
          <div className={`text-xs font-medium ${qtyColor}`}>
            {position.netQuantity}{" "}
            <span className="text-gray-500 font-normal">Qty</span>
          </div>
          <div className="text-sm md:text-base text-neutral-400 mt-1">
            {position.scriptId}
          </div>
          <div className="flex items-center mt-2">
            <span className="text-fuchsia-400 text-xs mr-3">
              {script?.exchange || ""}
            </span>
        ...
        <div className="flex flex-col items-end">
          ...
          <div className={`text-sm md:text-base text-base ${pnlColor}`}>
            {formatPrice(pnl)}
          </div>
          <div className="text-gray-400 text-xs mt-1">
            LTP {formatPrice(quote?.price)}
          </div>
        </div>
      </div>
      <div
