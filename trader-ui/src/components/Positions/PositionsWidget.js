...
const PositionsWidget = () => {
  ...
  useEffect(() => {
    ...
    positions.forEach((pos) => {
      ...
      const unsubscribe = eventBus.on(
        ...
        () => {
          setQuotes((prev) => ({
            ...prev,
            [scriptId]: quotesStore.getQuote(scriptId),
          }));
        },
      );
    ...
    return () => {
      Object.entries(quoteUnsubRef.current).forEach(
        ([scriptId, unsubscribe]) => {
          if (typeof unsubscribe === "function") unsubscribe();
        },
      );
      quoteUnsubRef.current = {};
  ...
  return (
    <div className="bg-dark-bg-2 rounded-lg p-3 flex-1 font-family-roboto relative">
      {/* Overlay Loader */}
      {loadingExitAll && (
        <div
        ...
        >
          ...
          <div className="text-orange-400 text-lg font-semibold mt-2">
            Exiting...
          </div>
        </div>
      )}
      ...
      <div className="bg-black text-center py-3 rounded mb-2 border border-neutral-700 mx-3">
        ...
        <div className={`${totalPnLColor} text-2xl`}>
