    ...
    >
      <div className="flex items-center px py-2 w-full">
        ...
        <div className="watchlist-item-price flex flex-col items-end">
          <div
            className={`watchlist-item-price-main text-sm md:text-base ${priceColor === "green" ? "text-price-green" : "text-price-red"}`}
          >
            {formatPrice(price)}
          </div>
      ...
      {!isMobile && (
        <div
          className={`w-full transition-all duration-300 ease-in-out overflow-hidden ${
            showSummary ? "max-h-[500px] opacity-100 mt-2" : "max-h-0 opacity-0"
          }`}
        >
