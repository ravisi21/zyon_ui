...
const WatchlistsWidget = () => {
  const [watchlists, setWatchlists] = useState(() =>
    watchlistsStore.getWatchlists(),
  );
  const [selectedWatchlistId, setSelectedWatchlistId] = useState(() =>
    watchlistsStore.getSelectedWatchlistId(),
  );
  ...
  useEffect(() => {
    const unsubscribeWatchlists = eventBus.on(
      EVENT_TYPES.WATCHLISTS_UPDATE,
      () => {
        setWatchlists(watchlistsStore.getWatchlists());
      },
    );
    const unsubscribeSelected = eventBus.on(
      EVENT_TYPES.WATCHLIST_SELECTED,
      () => {
  ...
  const selectedWatchlist = useMemo(() => {
    return (
      watchlists.find((wl) => wl.watchlistId === selectedWatchlistId) || null
    );
