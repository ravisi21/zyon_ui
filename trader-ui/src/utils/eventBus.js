class EventBus {
  ...
  on(eventType, listenerOrTopicId, maybeListener) {
    if (
      typeof listenerOrTopicId === "function" &&
      ...
      if (!this.topicEvents[eventType][topicId])
  ...
  off(eventType, listenerOrTopicId, maybeListener) {
    if (
      ...
      this.globalEvents[eventType] = this.globalEvents[eventType].filter(
        (l) => l !== listenerOrTopicId,
      );
    } else if (
      typeof listenerOrTopicId !== "function" &&
      typeof maybeListener === "function"
      ...
      const topicId = listenerOrTopicId;
      const listener = maybeListener;
      if (!this.topicEvents[eventType] || !this.topicEvents[eventType][topicId])
        return;
      this.topicEvents[eventType][topicId] = this.topicEvents[eventType][
        topicId
      ].filter((l) => l !== listener);
    }
  }
  ...
  emit(eventType, topicId) {
    ...
    if (
      ...
      this.topicEvents[eventType][topicId].forEach((listener) =>
        setTimeout(() => listener(), 0),
      );
    }
    // Always emit to global listeners
    if (this.globalEvents[eventType]) {
      this.globalEvents[eventType].forEach((listener) =>
        setTimeout(() => listener(), 0),
      );
    }
  }
...
export const EVENT_TYPES = {
  ...
  SHOW_USER_SIGNUP_UPDATE: "SHOW_USER_SIGNUP_UPDATE",
  ...
  SHOW_HOME_USER_LOGIN_UPDATE: "SHOW_HOME_USER_LOGIN_UPDATE",
  SHOW_HOME_USER_SIGNUP_UPDATE: "SHOW_HOME_USER_SIGNUP_UPDATE",
  SIGN_IN_FOLLOW_UP_UPDATE: "SIGN_IN_FOLLOW_UP_UPDATE",
